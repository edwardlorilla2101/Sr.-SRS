...
def get_ollama_response(input_text, no_words, blog_style, word_of_the_day, model_name="llama2:chat"):
    ...
    prompt = "Generate a HTML code without image tag for a blog post directly can generate professional blog post with html tags. The content of the blog should be based on the following prompt: "  + random.choice(prompts)
    try:
